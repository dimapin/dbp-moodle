# https://github.com/bitnami/containers/blob/main/bitnami/moodle/5.0/debian-12/rootfs/opt/bitnami/scripts/libapache.sh
...
apache_replace_htaccess_files() {
    ...
    # Default options
    local document_root="${APACHE_ROOT_DIR}/${app}"
    local overwrite="yes"
...
ensure_apache_app_configuration_exists() {
    ...
    export document_root="${APACHE_ROOT_DIR}/${app}"
...
EOF
    ...
    local template_name="app"
    [[ -n "$type" && "$type" != "php" ]] && template_name="app-${type}"
    local -r template_dir="${APACHE_ROOT_DIR}/scripts/apache/bitnami-templates"
